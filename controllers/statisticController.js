...
const getTopCustomers = (dateStart, dateEnd, limit) => {
  return $.ajax({
    url:
      "util/statistic.php?dateStart=" +
      dateStart +
...
const statistic4 = async () => {
  let dateStart = document.querySelector("#statistic-type4 .dateStart").value;
  let dateEnd = document.querySelector("#statistic-type4 .dateEnd").value;
  let limit = 5; // Số lượng khách hàng cần liệt kê
  ...
  if (new Date(dateStart) > new Date(dateEnd)) {
    customNotice(
      "fa-sharp fa-light fa-circle-exclamation",
      "Date start must be smaller or equal to date end!",
      3
    );
    return;
  }
