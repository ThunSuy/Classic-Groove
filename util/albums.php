...
switch ($_SERVER["REQUEST_METHOD"]) {
    case 'GET':
        switch ($_GET['action']) {
            ...
            case 'checkAlbumUsed':
                $albumID = $_GET['albumID'];
                $sql = "SELECT * FROM chitiethoadon WHERE album = " . $albumID;
                $result = $dp->excuteQuery($sql);
                if ($result->num_rows > 0) {
                    echo "used";
                } else {
        ...
        switch ($_POST['action']) {
            ...
            case 'addNewAlbum':
                ...
                $sql = "INSERT INTO album
                        VALUES(" . $albumID . ",'" . $albumName . "'," . $ablumPrice . ",'" . $albumDescribe . "','" . $albumImage . "','" . $albumArtist . "',1,99999," . $albumKind . ")";
                echo $sql;
                $result = $dp->excuteQuery($sql);
                break;
        }
    ...
    case 'DELETE':
        switch ($_GET['action']) {
            ...
            case 'deleteAlbum':
                ...
                // Xóa khỏi bảng trung gian baihat_album trước (nếu có)
